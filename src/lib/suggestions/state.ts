...
export function computeAthleteState(
  ...
  const typical_pace_flat = median(flatRuns.map((a) => a.pace_sec_per_km));
  const typical_pace_hilly = median(hillyRuns.map((a) => a.grade_adjusted_pace));
  ...
  return {
    ...
    longest_run_28d_km,
    typical_pace_flat,
    typical_pace_hilly: typical_pace_hilly || typical_pace_flat,
